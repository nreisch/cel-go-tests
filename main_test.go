...
func BenchmarkTest_Policy1_CompileEvaluate(b *testing.B) {
    env, _ := setupGlobalEnv()

    exp := "input.properties.contains('mode','standard')"
    jsonInputStr := `
    {
        "input": {
            "location" : "eastus",
    ...
    for i := 0; i < b.N; i++ {
        compile_and_evaluate(env, exp, jsonInputStr, 1)
    }
}

func BenchmarkTest_Policy1_Evaluate(b *testing.B) {
    env, _ := setupGlobalEnv()
    exp := "input.properties.contains('mode','standard')"
...
func BenchmarkTest_Policy100_CompileEvaluate(b *testing.B) {
    env, _ := setupGlobalEnv()
    exp := "input.properties.contains('mode','standard')"
...
func BenchmarkTest_Policy100_Evaluate(b *testing.B) {
    env, _ := setupGlobalEnv()
